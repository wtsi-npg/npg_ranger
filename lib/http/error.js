 ...
 *   const DataAccess = require('../lib/http/error.js');
 *   // Create a new object.
...
class HttpError {
  ...
  constructor(response, code, message, verbose) {
    ...
    this.errorType = HTTP_ERROR_TYPES[code + ''];
    if (!this.errorType) {
    ...
    if (verbose) {
      console.log(`Server error ${code}: ${this.message}.`);
    }
  }
   ...
   * according to the GA4GH specification.
   * 
   * If the headers have been set and chunked trunsfer encoding is used
   * and the data truncation trailer is declared in the headers, this
   * trailer is set to mark the data truncated.
   */
  setErrorResponse() {
    if (!this.response.headersSent) {
      ...
      response.writeHead(code, m, {
        'Content-Length': jerr.length,
        'Content-Type':   'application/json'
                        });
      response.write(jerr);
    } else {
      trailer.setDataTruncation(this.response, 0);
    }
  }
}
