...
var procJSON = ( data, headers ) => {
  ...
  for (let i = 0; i < htsget.urls.length; i++ ) {
    let uri;
    let individual_headers;
    if ( htsget.urls[i].url ) {
      uri = htsget.urls[i].url;
      if ( htsget.urls[i].headers ) {

        // Merge headers, give priority to new headers provided
        individual_headers = _mergeHeaders(htsget.urls[i].headers, headers);
      }
    } else {
      throw new Error( 'Malformed JSON redirect, missing url field in: ' +
                       JSON.stringify(htsget.urls[i]) );
    }
    uris[i] = uri;
    headers4uris[i] = individual_headers;
  }
