...
class RangerRequest extends EventEmitter {
   ...
   * @param {boolean} [async] - The request will be async, by default <code>true</code>.
   *                            If provided, it will be enforced to be <code>true</code>
   */
  ...
  send( payload ) {
    ...
    console.log(self._requestOptions.url);
  ...
  _procJSON( data ) {
    ...
    console.log('Response with JSON: ' + decdStr);
    var uris         = [];
    var headers4uris = [];
    var uriPromises  = [];
    ...
    for (let i = 0; i < jsonResponse.urls.length; i++ ) {
      ...
      if ( jsonResponse.urls[i].url ) {
        ...
        console.log('Found URI from JSON ' + uri);
