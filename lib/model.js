...
class RangerModel {
   ...
   */
  getFile() {
    const view = child.spawn(SAMTOOLS_COMMAND, this.stViewAttrs(this.query));
  ...
  mergeFiles() {
    ...
    var attrs;
    try {
      attrs = this.stMergeAttrs(this.query);
    } catch (ex) {
    ...
    const markdup = child.spawn(BBB_MARKDUPS_COMMAND, this.bbbMarkDupsAttrs());
