 ...
 *   // Authorise a user for access for data with data access group ids
     // given by the second attribute. Group ids should be strings or arrays
     // of strings.
 *   da.authorise('alice', ["6", "34"]);
 *   // or
 *   da.authorise('alice', ["6", ["1", "7"]]);
 *   // or its equivalent with all individual access groups as arrays
 *   da.authorise('alice', [["6"], ["1", "7"]]);
 *
...
class DataAccess extends EventEmitter {
  ...
  authorise(identifier, accessGroups) {
    ...
    let accessGroupsList = accessGroups.map( file_acc_grps => {
      return ( file_acc_grps.constructor === Array ) ? file_acc_grps
                                                     : new Array(file_acc_grps);
    });

    LOGGER.debug('ACCESS GROUP IDS: ' + JSON.stringify(accessGroupsList));
    let auth_server = config.provide().get('auth_server');
    let authurl = auth_server.url;
    let reqbody = {groups: accessGroupsList};
    let reqopts = {};
