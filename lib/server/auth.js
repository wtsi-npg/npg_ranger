...
class DataAccess extends EventEmitter {
  ...
  authorise(identifier, accessGroups) {
    ...
    if ( authurl.startsWith('https://') ) {
      let auth_cert = config.provide().get('auth_cert');
      if ( auth_cert ) {
        reqopts.cert = fs.readFileSync(auth_cert);
      }
      let auth_key = config.provide().get('auth_key');
      if ( auth_key ) {
        reqopts.key = fs.readFileSync(auth_key);
        let auth_passphrase = config.provide().get('auth_key_passphrase');
        if ( auth_passphrase ) {
          reqopts.passphrase = auth_passphrase;
        }
      }
      let auth_ca = config.provide().get('auth_ca');
      if ( auth_ca ) {
        reqopts.ca = fs.readFileSync(auth_ca);
      }
    }
