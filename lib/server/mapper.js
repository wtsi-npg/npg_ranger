...
class DataMapper extends EventEmitter {
  ...
  getReferenceInfo(query, host) {
    ...
    // The in operator is applies like this, on two or more expressions where it's only going to be in one field.
        ...
        if (err) {
          ...
          if (doc != null) {
            if (typeof doc.end === "undefined") {
              ...
              this.emit(DB_ERROR_EVENT_NAME, dbErrorMessage + 1);
            }
            outputData[doc.referenceName] = doc.end;
          } else {
            cursor.close();
            if (Object.entries(outputData).length > 0) {
              ...
              if (Object.entries(outputData).length < refCount) {
                LOGGER.error(`Requested DB entry does not exist 
                             for ${query.multiregions_key[refCount - 1].referenceName}`);
                this.emit(DB_ERROR_EVENT_NAME, dbErrorMessage + 2);
              }
              ...
              this.emit(DB_ERROR_EVENT_NAME, dbErrorMessage + 3);
            }
          }
        }
      });
