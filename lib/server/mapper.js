  ...
  getFileInfo(query, host) {
    ...
    let filterArr = [
      ...
      },
      {
        name: 'avh.alignment',
        val: query.alignment,
        inv: query.alignment_not,
        default: '1',
        blacklist: []
      },
      {
        name: 'avh.alt_target',
        val: query.alt_target,
        inv: query.alt_target_not,
        default: null,
        blacklist: []
      },
      {
        name: 'avh.alt_process',
        val: query.alt_process,
        inv: query.alt_process_not,
        default: null,
        blacklist: []
      },
      {
        name: 'avh.alignment_filter',
        val: query.alignment_filter,
        inv: query.alignment_filter_not,
        default: null,
        blacklist: []
      }
    ];
    ...
    if (a) {
      ...
      for (let filter of filterArr) {
        ...
        if ((filter.val !== undefined && filter.inv !== undefined)
            || (filter.blacklist && (filter.val in filter.blacklist))) {

          this.emit(NO_DATA_EVENT_NAME, 'Invalid query');
          break;
        } else if (filter.val !== undefined) {
          if (filter.val === 'undef') {
            dbquery.$and.push({[filter.name]: {$exists: false} });
          } else if (filter.val !== '') {
            dbquery.$and.push({[filter.name]: filter.val});
          }
        } else if (filter.inv !== undefined) {
          if (filter.inv === 'undef') {
            dbquery.$and.push({ [filter.name]: {$exists: true} },
                              { [filter.name]: {$nin: filter.blacklist} });
          } else if (filter.inv !== '') {
            dbquery.$and.push({ [filter.name]: {$ne: filter.inv} },
                              { [filter.name]: {$nin: filter.blacklist} });
          }
