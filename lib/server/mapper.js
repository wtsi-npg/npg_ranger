...
class DataMapper extends EventEmitter {
  ...
  getReferenceInfo(query, host) {
    ...
    if (listOfChr.length) {
      if (listOfChr.length === 1) {
        dbquery.referenceName = listOfChr[0];
      ...
      cursor.each( (err, doc) => {
        if (err) {
          ...
          this.emit(ERROR_EVENT_NAME, 'DB error');
        } else {
          if (doc != null) {
            ...
            if (outputData) {
              ...
              this.emit(NO_DATA_EVENT_NAME, "No DB entries returned, one expected, DB error");
            }
          }
        }
      });
      ...
      this.emit(DATA_READY_EVENT_NAME, "no data"); // No data event name causes an error, how to handle this?
    }
  }
