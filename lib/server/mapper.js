...
class DataMapper extends EventEmitter {
  ...
  _fixReference(dbref) {
    // Reference path in database is often incorrect. So use only
    // sample name and filename from database's path, and instead
    // substitute in actual path.
    let ref   = dbref.split('/').slice(-5);
    ref[ref.length - 2] = 'fasta';
    ref                 = ref.join('/');
    let options = config.provide();
    let rootref = options.get('references');
    if (rootref) {
      return path.resolve('/' + rootref + '/' + ref);
    }
        ...
        if (doc != null) {
          ...
          if (files.length === 0) {
            ...
            let data = files.map( (f) => {
              ...
              if (f.avh.reference) {
                d.reference = this._fixReference(f.avh.reference);
              }
              LOGGER.debug('Using reference file ' + d.reference);
              return d;
            });
