...
module.exports = (processes, onSuccessClbk, onFailureClbk) => {
  ...
  function createPromise(p) {
    return new Promise(
      function(resolve, reject) {
        ...
        var rejectAndKill = function(code) {
          p.npgIsClosed = true;
          if (code) {
            reject(code);
            killProcesses();
        ...
        p.once('close', function(code, signal) {
          LOGGER.info(p.title + ' sent event close');
          rejectAndKill(code || signal);      
        });
        p.once('exit', function(code, signal) {
          LOGGER.info(p.title + ' sent event exit');
          // if process exited with a non-zero code, error occurred
          rejectAndKill(code || signal);
        });
        p.on('error',        (err) => {
          LOGGER.error(p.title + ' emitted error ' + err);
          reject(err);
        });
  ...
  function schedule(destination, processTimeoutGrace) {
    ...
    try {
      ...
      // If it happened after client disconnect, had_error is false.
      socket.on('close', (had_error) => {
        LOGGER.debug('socket close event');
        LOGGER.debug(had_error
                   ? 'error occurred in socket close'
                   : 'no error occurred in socket close');
        if (had_error) {
          LOGGER.error(
            'Transmission error - response socket closed - killing processes');
          ...
          // Sometimes client may disconnect but had_error will remain false.
          // So, after a grace period, make sure that all processes have
          // been killed.
          setTimeout(function() {
            killProcesses();
          }, processTimeoutGrace);
  ...
  return {
     ...
     * @param processTimeoutGrace - time period to wait before killing any remaining processes in milliseconds
     */
    run: (destination, processTimeoutGrace) => {
      if (!destination) {
      ...
      validateInput();
      registerOutcomes();
      schedule(destination, processTimeoutGrace);
    },
  };
};
