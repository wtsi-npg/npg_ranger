  ...
  function schedule(destination) {
    ...
    try {
      ...
      socket.on('close', () => {
        ...
        // After some waiting some time, kill any processes which are still alive
        setTimeout(function() {
          processes.forEach(function(p) {
            if (!p.npgIsClosed) {
              LOGGER.debug('killing with timeout');
              p.kill();
            }
          });
        }, config.provide().get('timeout') * 1000); // * 1000 to convert seconds to ms
      });
    } catch (e) {}
