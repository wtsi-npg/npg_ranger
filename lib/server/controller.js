...
class RangerController {
  ...
  _serverUrlFromRequest(protocol) {
  ...
  _serverUrlFromProxy(knownProxies) {
  ...
  _redirect(id, query) {
    ...
    if (url) {
      ...
      let plist = c.get('proxylist');
      let serverUrl;
      try {
        serverUrl = plist
          ? this._serverUrlFromProxy(plist)
          : this._serverUrlFromRequest(c.get('protocol'));
      } catch (err) {
        this.errorResponse(this.response, 422, err.message);
      }
      if (serverUrl) {
  ...
  handleRequest() {
    ...
    if (!user.username && !options.get('skipauth')) {
