...
class RangerController {
  ...
  _getData(query, authtype, cback) {
    ...
    var dm1 = new DataMapper(this.db);
    var dm2 = new DataMapper(this.db);
    this._setDefaultListeners(dm1);
    ...
    var p2 = new Promise((resolve, reject) => {
      dm2.once('data', (data) => {
        dm2.removeAllListeners();
        // Process data, add end region to every chr without one
        // then resolve, reject if any issues? Insta-resolve if "no data" etc is sent.
        if (data === "no data") { // Like this?
          resolve();
        } else {
          try {
            console.log("STARTING WITH ORIGINAL QUERY: ");
            console.log(query);
            console.log("AND THE DATA DELIVERED FROM FUNCTION: ");
            console.log(data);
            for (let i = 0; i < query.multiregions_key.length;  i++) {
              if (!(query.multiregions_key[i].end)) {
                query.multiregions_key[i].end = data[query.multiregions_key[i].referenceName];
              }
            }
            resolve();
          } catch (e) {
            // Should a this.errorResponse be created, like in dm1?
            reject(e);
          }
        }
      });
    ...
    dm1.getFileInfo(query, host);

    p1.then(() => {
      // We want to run dm2/the second process after the first one's finished,
      // so that the query changes have been made and are accessible within.
      this._setDefaultListeners(dm2);
      dm2.getReferenceInfo(query, host);
      p2.then(() => {
