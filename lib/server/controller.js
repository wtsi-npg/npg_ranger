...
class RangerController {
  ...
  _getData(query, authtype, cback) {
    ...
    dm.once('data', (data) => {
      ...
      if (options.get('skipauth')) {
        ...
        da.authorise(authinput, data.map( (d) => {return d.accessGroup;} ));
      }
    });
    dm.getFileInfo(query, host);
  }
  ...
  handleRequest() {
    ...
    let authtype = authmatch[1] || null;

    this._user = {};
    if (options.get('skipauth')) {
      // noop - don't need authorisation
    } else if (authtype === constants.AUTH_TYPE_USER) {
      ...
      return this.errorResponse(this.response, 401, 'Authtype must be specified in url');
    } else {
      return this.errorResponse(this.response, 401, 'Unrecognised authtype');
    }

    // Finished handling authtype, so strip `/${authtype}` from url
