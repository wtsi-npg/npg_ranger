...
class RangerController {
   ...
   * @param skipAuth - boolean flag, optional, if true instructs to
   *                   skip authorisation
   * @param noStrict - boolean flag, if true instructs to disable
   *                   strict mode. Server will not generate VCF
   *                   output while strict mode is disabled.
   * @param processTimeoutGrace - integer, number of milliseconds to
   *                   wait before killing any processes still running
   *                   after socket close.
   */
  constructor(request, response, db, tmpDir, skipAuth, noStrict, processTimeoutGrace) {
    ...
    LOGGER.info(this.noStrict
            ? 'Strict mode disabled'
            : 'Running in strict mode');

    this.processTimeoutGrace = parseInt(processTimeoutGrace || 600000);
    assert(!isNaN(this.processTimeoutGrace),
           'Process\' grace period must be a number');
    assert(this.processTimeoutGrace > 0 && this.processTimeoutGrace < 2147483647,
  ...
  _setupPipeline(query) {
    ...
    try {
      let model = new RangerModel(this.tmpDir, this.processTimeoutGrace);
  ...
  _getData(user, host, query) {
    ...
    dm.once('data', (data) => {
      ...
      // use the same reference file. So, when running in strict
      // mode, throw an error if the reference file names do not match.
      // Remember that multiple paths may hold same .fa file, so test
      // file name.
      if (!this.noStrict) {
        var refFile = path.basename(data[0].reference);
        var allMatch = data.every(function(element) {
          return path.basename(element.reference) === refFile;
        });
        assert(allMatch, 'All files must use the same reference file in strict mode.');
      }
  ...
  handleRequest(host) {
    ...
    if (q.format === 'VCF' && this.noStrict) {
      return this.errorResponse(this.response, 422,
        'Invalid request: ' +
        'cannot produce VCF files while server is not in strict mode');
    }
