...
const jsonSchema  = require('@prantlf/jsonlint/lib/validator'); //
...
let _processQuery = (postTmp) => {
  let newQuery = {};
  newQuery.format = postTmp.format;
  newQuery.accession = postTmp.accession;
  if (typeof postTmp.regions !== 'undefined') {
    newQuery.referenceName = postTmp.regions[0].referenceName;
    newQuery.start = typeof postTmp.regions[0].start !== 'undefined' ? postTmp.regions[0].start : undefined;
    newQuery.end = typeof postTmp.regions[0].end !== 'undefined' ? postTmp.regions[0].end : undefined;
  }
  return postTmp;
}
...
let _parsePostBody = async (request) => {
  let parsePromise = new Promise((resolve, reject) => {
    ...
    request.on('data', (chunk) => {
      ...
      console.log("THEDATA TO PARSE ISSSS: ");
      console.log(JSON.parse(chunk));
      let validate = jsonSchema.compile(JSON.stringify(htsSchema));
      try {
      ...
      try {
        ...
        reject("Does not match schema");
      }
      // TODO - reimplement all as a function
      // --
      console.log('After parsing and linting');
      console.log(postTmp);
      resolve(postTmp);
    });
...
let _parsePostBodyError = (response, e, path, errorResponse) => {
  ...
  let g = e.toString().match(/Duplicate key: "(?<attribute>.+)"/);
  if (g) {
    return errorResponse(response, 400, "Invalid request: multiple values for attribute '" +
                         g.groups.attribute + "'", ServerHttpError.INVALID_INPUT);
  }
  // TODO find proper error message for 400?
  return errorResponse(response, 400, 'Invalid Request: ' + e.toString());
};
...
class RangerController {
  ...
  _ga4ghToDirectUrl(id, query) {
    ...
    let reference2RegionString = (queryReference) => {
      let regionString = '';
      regionString = queryReference.referenceName;
      ...
      console.log("Region console log -------------------------------------");
      console.log();
      return regionString;
    };

    if (query.referenceName) {
      let region = reference2RegionString(query); 
      url += ('&region=' + region);
    } else if (query.regions) {
      let region = reference2RegionString(query.regions[0]);
      url += ('&region=' + region);
    } else {
  ...
  async handleRequest() {
    ...
    if (this.request.method === 'POST') {
      try {
        q = await _parsePostBody(this.request);
        console.log('==== here ====');
        console.log(q);
      } catch (error) {
        return _parsePostBodyError(this.response, error, path, this.errorResponse);
        // console.log("Error is ----: " + error.toString());
        // if (error.toString().match('multiple values for attribute')) {
        // return this.errorResponse(this.response, 400, error, ServerHttpError.INVALID_INPUT);
        // }
        // TODO find proper error message for 400
        // return this.errorResponse(this.response, 400, 'Invalid Request: ' + path);
      }
    } else {
      q = urlObj.query;
    }
    console.log("The value of q is: ");
    console.log(q);
    ...
    let duplicateAttr = (query) => {
      return Object.keys(query).find(
        (element) => { return element !== 'regions' && typeof query[element] === 'object'; }
      );
    };
    ...
    console.log('What is in the query');
    console.log(q);
