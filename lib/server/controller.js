...
const LOGGER       = require('../logsetup.js').LOGGER;
...
class RangerController {
  ...
  _setDefaultListeners( dm ) {
    ...
    dm.once('dberror', ( message ) => {
      dm.removeAllListeners();
      this.errorResponse(this.response, 500, message);
    });
  }
      ...
      dm2.once('data', (data) => {
        ...
        if (data == null) {
          ...
          try {
            ...
            this.errorResponse(this.response, e.code, e.message);
            reject(e);
          }
        }
      });
