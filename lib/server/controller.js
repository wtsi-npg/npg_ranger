...
class RangerController {
  ...
  _setDefaultHeaders() {
    let port = config.get('port') || 'unknown';
    if ( !this.skipAuth && typeof port == 'number' && config.get('cors') ) {
      this.response.setHeader('Access-Control-Allow-Origin', '*');
      this.response.setHeader('Access-Control-Allow-Methods', 'GET,OPTIONS');
      this.response.setHeader('Access-Control-Allow-Headers', 'TE');
    }
  }
