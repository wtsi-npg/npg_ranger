...
class RangerController {
   ...
   *                   skip authorisation
   * @param unsafe   - boolean flag, if true instructs to disable
   *                   safe mode. Server will not generate VCF
   *                   output while safe mode is disabled.
   */
  constructor(request, response, db, tmpDir, skipAuth, unsafe) {
    ...
    this.unsafe = typeof unsafe === 'boolean'
                      ? unsafe
                      : !!options.get('unsafe');
  }
  ...
  _getData(user, host, query) {
    ...
    dm.once('data', (data) => {
      ...
      // use the same reference file. So, when running in safe
      ...
      if (!this.unsafe) {
        ...
        assert(allMatch, 'All files must use the same reference file in safe mode.');
      }
  ...
  handleRequest(host) {
    ...
    if (q.format === 'VCF' && this.unsafe) {
      return this.errorResponse(this.response, 422,
        'Invalid request: ' +
        'cannot produce VCF files while server is not in safe mode');
    }
