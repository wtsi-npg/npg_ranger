...
let checkStartEnd = (regions) => {
  ...
  regions.forEach ( region => {
    ...
    region.end = region.end || 30000; // Temp
  });
  return;
};
...
class RangerModel {
  ...
  _stMergeAttrs(query, dir) {
    ...
    if (query.region) {
      ...
      // Changes the value of query due to JS shenanigans.
      checkStartEnd(query.multiregions_key);
      let bedString = buildBedFile(query.multiregions_key);
      let fileName = Date.now() + '.bed';
      fse.ensureDirSync(dir + '/bedFiles');
      fse.writeFileSync(dir + '/bedFiles/' + fileName, bedString);
      attrs.push('-L');
      attrs.push(dir + '/bedFiles/' + fileName);
    }
