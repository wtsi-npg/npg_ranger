...
class ServerFactory extends EventEmitter {
  ...
  startServer() {
    ...
    if ( startssl ) {
      let key = options.get('secure_key');
      let passphrase = options.get('secure_passphrase');
      let cert = options.get('secure_cert');
    ...
    [ 'SIGTERM', 'SIGINT', 'SIGHUP' ].forEach( ( sig ) => {
      process.on( sig, () => {
        if ( typeof server.shutdown === 'function' ) { // TODO check if https has it
          server.shutdown( () => {
            process.exit(0);
          });
        } else {
