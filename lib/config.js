...
var fromCommandLine = () => {
  ...
  opt.options.set('config_ro', true);
  return opt.options;
};
 ...
 *                          had not yet been initialized or
...
var provide = function(generateConfigs) {
  if (generateConfigs) {
    if (options && options.get('config_ro')) {
    ...
    if (opts.get('anyorigin')) {
      ...
      opts.set('originlist', olString.split(',').map((el) => {
        ...
        try {
          ...
          if (urlObj.pathname && urlObj.path &&
            ((urlObj.pathname.length > 1 || urlObj.path !== urlObj.path))) {
