...
function adjustOptions(options) {
  ...
  var setPort = () => {
    ...
    if (Number.isInteger(Number.parseInt(options.get('port')))) {
      options.set('port', Number.parseInt(options.get('port')));
    }
  };
  ...
  var setACAOrigin = () => {
    ...
    if (options.get('anyorigin')) {
      if (typeof options.get('anyorigin') != 'boolean') {
        throw "'anyorigin' should be a boolean value";
      }
      if (options.get('originlist')) {
        throw "'anyorigin' and 'originlist' options cannot both be set";
      }
      if (!options.get('skipauth')) {
        throw 'anyorigin option cannot be used if authorization is performed';
      }
    } else if (options.get('originlist')) {
      let olString = options.get('originlist');
      if (typeof olString != 'string') {
        throw "'originlist' should be a comma-separated string";
      }
      let re = new RegExp(/\*/);
      if (re.test(olString)) {
        throw "'originlist' string should not contain the wild card character *";
      }
      let l = olString.split(',');
      l.forEach((el) => {
        try {
          urlParser.parse(el);
        } catch (e) {
          throw 'Invalid URL ${el} in a list of allowed origin URLs: ' + e;
        }
      });
      options.set('originlist', l);
    }
  };
